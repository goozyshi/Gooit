...
const status = [{name: '未开始', color: '#09AAFF'}, {name: '进行中', color: '#E2943B'}, {name: '已完成', color: '#1db954'}]
  ...
  render() {
    ...
    return (
      ...
      { title === '更改状态' && status.map((item)=>{
        return (
          <TouchableOpacity style={styles.task_container} key={item.name} onPress={()=>this.updateState(item.name)}>
